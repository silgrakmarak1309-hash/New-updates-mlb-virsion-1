...
class BazaarViewModel(
    ...
    val banners: List<BannerItem> = listOf(
        BannerItem(
            id = "b1",
            title = "Sell Faster with PRO",
            subtitle = "Get up to 10x more inquiries on your listings",
            tag = "FEATURED",
        ...
        BannerItem(
            id = "b2",
            title = "Trusted Local Community",
            subtitle = "Buy & sell directly with verified local buyers & sellers",
            tag = "SAFE & VERIFIED",
            imageUrl = "https://images.unsplash.com/photo-1526170375885-4d8ecf77b99f?w=800&auto=format&fit=crop&q=80",
            bgGradientStart = 0xFF0F766E,
            bgGradientEnd = 0xFF115E59
        ),
        BannerItem(
            id = "b3",
            title = "Upgrade to PRO Membership",
            subtitle = "Highlight your ads with golden badge & top ranking",
            tag = "PRO UPGRADE",
    ...
    private suspend fun initUserProfileState() {
        try {
            val profiles = repository.firebaseService.fetchUsers()
            val currentEmail = userEmail.value.trim().lowercase()
            val myProfile = profiles.find { it.email.trim().lowercase() == currentEmail }
            if (myProfile != null) {
                isProUser.value = myProfile.isPro
                if (myProfile.role == "admin" || myProfile.role == "super_admin") {
                    isAdminUser.value = true
                }
            }
        } catch (e: Exception) {
            // Keep current offline state
        }
    }

    fun activateFreeMonthPro() {
        isProUser.value = true
        showSnackbar("🎉 Free 30-Day PRO Membership Activated!")
    }
    ...
    fun syncFirebaseData(silent: Boolean = false) {
        viewModelScope.launch {
            ...
            try {
                ...
                if (!silent) {
                    if (success) {
                        showSnackbar("☁️ Connected & Synced with Supabase Production Database")
                    } else {
    ...
    // -------------------------------------------------------------
    // POSTING & MODERATION (Moderated as 'pending' in Supabase)
    // -------------------------------------------------------------
    fun postNewAd(
        title: String,
        categoryId: String,
        ...
        viewModelScope.launch {
            val newListing = ListingEntity(
                ...
                status = "active", // Locally visible, pending on Supabase moderation
            ...
            showSnackbar("🎉 Ad submitted for moderation on Meri Local Bazaar!")
    ...
    // -------------------------------------------------------------
    // USER MANAGEMENT & PROFILES SYNC (Supabase profiles table)
    // -------------------------------------------------------------
    ...
    // -------------------------------------------------------------
    // SETTINGS MANAGEMENT (Supabase settings table)
    // -------------------------------------------------------------
